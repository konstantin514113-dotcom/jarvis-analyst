...
def get_news_sentiment(symbol):
    """Get news sentiment from LunarCrush for a symbol"""
    try:
        ...
        d = r.json().get("data", {})
        sentiment = d.get("sentiment", 3)  # 1-5 scale, 3 = neutral
        galaxy_score = d.get("galaxy_score", 50)
        alt_rank = d.get("alt_rank", 500)
        bullish = sentiment >= 3.5 and galaxy_score >= 50
        return {
        ...
        return {"sentiment": 3.0, "galaxy_score": 50, "alt_rank": 500, "bullish": True}
...
def analyze_pair(symbol):
    ...
    score += 10 if news["bullish"] else -5
    return {**t, "rsi": rsi_15m, "rsi_1h": rsi_1h, "macd": macd_15m, "macd_1h": macd_1h,
            ...
            "sentiment": news["sentiment"], "galaxy_score": news["galaxy_score"], "news_bullish": news["bullish"]}
...
.confirm{font-size:10px;color:#00e676;margin-bottom:8px}
.news-row{font-size:10px;margin-bottom:12px;display:flex;gap:8px}
.news-bull{color:#00e676}.news-bear{color:#ff5252}.news-neu{color:#58a6ff}
...
function newsLabel(s, bullish) {
  if (bullish) return '<span class="news-bull">📰 Новости: бычьи · sentiment '+s+'</span>';
  if (s < 2.5) return '<span class="news-bear">📰 Новости: медвежьи · sentiment '+s+'</span>';
  return '<span class="news-neu">📰 Новости: нейтральные · sentiment '+s+'</span>';
}
...
function renderCards(sigs){
  ...
  document.getElementById('cards').innerHTML = sigs.map((s,i)=>{
    ...
    return '<div class="card '+rc+'">'+
      ...
      '<div class="news-row">'+newsLabel(s.sentiment||3.0, s.news_bullish)+'</div>'+
      '<div class="prices">'+
